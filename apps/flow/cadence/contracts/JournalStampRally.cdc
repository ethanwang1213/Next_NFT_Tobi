...
access(all) contract JournalStampRally: NonFungibleToken {
    ...
    access(all) resource NFT: NonFungibleToken.NFT {
        ...
        access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection} {
            return <- JournalStampRally.createEmptyCollection(nftType: Type<@JournalStampRally.NFT>())
        }
    }
    ...
    access(all) resource Collection: NonFungibleToken.Collection, CollectionPublic {
        ...
        access(all) view fun getSupportedNFTTypes(): {Type: Bool} {
            ...
            supportedTypes[Type<@JournalStampRally.NFT>()] = true
            return supportedTypes
        }

        access(all) view fun isSupportedNFTType(type: Type): Bool {
            return type == Type<@JournalStampRally.NFT>()
        }
        ...
        switch viewType {
            case Type<MetadataViews.NFTCollectionData>():
                let collectionData = MetadataViews.NFTCollectionData(
                    ...
                    publicCollection: Type<&JournalStampRally.Collection>(),
                    publicLinkedType: Type<&JournalStampRally.Collection>(),
    ...
    access(all) resource Minter {
        access(all) fun mintTo(creator: Capability<&{NonFungibleToken.Receiver}>, metadata: {String:String}): UInt64 {
            let id = JournalStampRally.totalSupply
            let meta = {
                "name": metadata["name"] ?? "",
                "description": metadata["description"] ?? "",
                "metaURI": "https://nft.tobiratory.com/journalstamprally/metadata/".concat(id.toString())
            };
    ...
    init() {
        ...
        if self.account.storage.borrow<&JournalStampRally.Collection>(from: JournalStampRally.collectionStoragePath) == nil {
            self.account.storage.save(<- create Collection(), to: self.collectionStoragePath)
            let cap: Capability = self.account.capabilities.storage.issue<&JournalStampRally.Collection>(self.collectionStoragePath)
