...
exports.checkReward = functions.region(REGION).https.onCall(async (data, context): Promise<StampRallyResultType> => {
  ...
  const correctStampEntry = strictEntries(TPF2023_STAMP_RALLY_KEYWORDS).find(v => v[1]===data.keyword);
  ...
  const mintStatusMap: MintStatusData = user?.mintStateTpf2023StampRally;
  const tpf2023StampRallyMap = mintStatusMap?.tpf2023StampRally
  if (tpf2023StampRallyMap) {
    ...
    // スタンプをコンプリートしたかチェック
    isStampCompleted = (() => {
      const requiredKeys = ["g0", "g1Alpha" ,"g1Beta", "g1Gamma", "g1Delta"]
